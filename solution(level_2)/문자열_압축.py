...
def pattern_compresser(gap,s):
    cnt = 1
    splited = [s[idx:idx+gap] for idx in range(0, len(s), gap)]
    new_s = ""
    pattern = splited[0]
    for word in splited[1:]:
        if word == pattern:
            ...
            new_s = new_s+pattern if cnt == 1 else new_s+str(cnt)+pattern
            pattern = word
            cnt = 1
    return new_s+pattern if cnt == 1 else new_s+str(cnt)+pattern

def solution(s):
    gap = 2
    mini = (1,len(pattern_compresser(1,s)))
    while gap <= (len(s)/2):
        temp = (gap,len(pattern_compresser(gap,s)))
        mini = min(mini,temp,key= lambda x:x[1])
        gap += 1
    return mini[1]
