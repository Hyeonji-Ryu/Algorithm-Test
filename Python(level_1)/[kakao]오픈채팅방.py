...
# [닉네임]님이 나갔습니다.
# 채팅방에서 닉네임을 변경하는 방법은 다음과 같이 두 가지이다. 채팅방을 나간 후, 새로운 닉네임으로 다시 들어간다. 채팅방에서 닉네임을 변경한다.
# 닉네임을 변경할 때는 기존에 채팅방에 출력되어 있던 메시지의 닉네임도 전부 변경된다. 예를 들어, 채팅방에 Muzi와 Prodo라는 닉네임을 사용하는 사람이 순서대로 들어오면 채팅방에는 다음과 같이 메시지가 출력된다.
...
def solution(record):
    splited = [log.split()[:2] for log in record]
    ans = []
    nick_names = {}
    lists = list(filter(lambda x: len(x) != 1, [log.split()[1:] for log in record]))

    for name in lists:
        if name[0] in nick_names.keys() :
            nick_names[name[0]] = name[1]
        else:
            nick_names[name[0]] = name[1]
    
    for log in splited:
        if log[0] == 'Enter':
            ans.append(f"{nick_names[log[1]]}님이 들어왔습니다.")
        elif log[0] == 'Leave':
            ans.append(f"{nick_names[log[1]]}님이 나갔습니다.")
        else:
            pass
